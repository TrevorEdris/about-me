...
var (
	//go:embed templates/components/* templates/emails/* templates/layouts/* templates/pages/* templates/*
	TemplateFS embed.FS
)
...
func NewTemplateRenderer(cfg *config.Config) *TemplateRenderer {
	...
	t.templatesPath = filepath.Join(filepath.Dir(d), "embedded", config.TemplateDir)
...
func (t *TemplateRenderer) Parse(cacheGroup, cacheID, baseName string, files []string, directories []string) error {
	...
	if _, err := t.Load(cacheGroup, cacheID); err != nil || t.config.App.Environment == config.EnvLocal {
		...
		parsed := template.New(baseName + config.TemplateExt).Funcs(t.funcMap)

		// Parse all files provided
		if len(files) > 0 {
			for k, v := range files {
				f := fmt.Sprintf("%s/%s%s", config.TemplateDir, v, config.TemplateExt)
				files[k] = f
			}

			parsed, err = parsed.ParseFS(TemplateFS, files...)
			if err != nil {
				return err
			}
		}
		...
		for _, dir := range directories {
			// List files in the dir using TemplateFS/dir
			tmplFiles, err := fs.ReadDir(TemplateFS, "templates/"+dir)
			if err != nil {
				return fmt.Errorf("failed to read dir from embedded templateFS: %w", err)
			}

			// Build list of files in the dir
			files := []string{}
			for _, tmpl := range tmplFiles {
				f := fmt.Sprintf("%s/%s/%s", config.TemplateDir, dir, tmpl.Name())
				files = append(files, f)
			}

			// Parse the template files in the dir
			parsed, err = parsed.ParseFS(TemplateFS, files...)
			if err != nil {
				return fmt.Errorf("failed to parse list of files from embedded templateFS: %w", err)
			}
		}
